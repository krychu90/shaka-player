...
shaka.media.MediaSourceEngine = class {
  ...
  async init(streamsByType, forceTransmuxTS) {
    ...
    for (const contentType of streamsByType.keys()) {
      ...
      if (contentType == ContentType.TEXT) {
        ...
        if (this.sourceBuffers_[contentType] &&
            this.canChangeType_(contentType)) {
          enqueueOperations.push(this.enqueueOperation_(contentType, () => {
            this.sourceBuffers_[contentType].changeType(mimeType);
    ...
    if (enqueueOperations.length > 0) {
      const p = Promise.all(enqueueOperations);
      for (const contentType of streamsByType.keys()) {
        if (!this.sourceBuffers_[contentType].updating) {
          this.onUpdateEnd_(contentType);
        }
      }
      await p;
   ...
   * @private
   */
  canChangeType_(contentType) {
    return typeof this.sourceBuffers_[contentType].changeType === 'function';
  }
