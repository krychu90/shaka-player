...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async onLoad_(has, wants) {
    ...
    this.switchVariant_(
        initialVariant, /* fromAdaptation= */ true, /* clearBuffer= */ false,
        /* safeMargin= */ 0);
   ...
   */
  getCurrentVariant() {
    return this.manifest_ && this.streamingEngine_ ?
   ...
   * @private
   */
  async switchVariant_(variant, fromAdaptation, clearBuffer, safeMargin) {
    ...
    if (!currentVariant || (
      this.isCompatibleCodec_(currentVariant.video, variant.video) &&
      this.isCompatibleCodec_(currentVariant.audio, variant.audio)
    )) {
   ...
   * @private
   */
  isCompatibleCodec_(s0, s1) {
    if (!s0 || !s1) {
      return true;
    }
    return s0.codecs.split('.')[0] === s1.codecs.split('.')[0];
  }
