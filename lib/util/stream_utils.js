...
shaka.util.StreamUtils = class {
  ...
  static chooseCodecsAndFilterManifest(manifest, preferredAudioChannelCount) {
    ...
    function variantCodecs(variant) {
      ...
      let videoCodec = '';
      if (variant.video) {
        videoCodec = MimeUtils.getCodecBase(variant.video.codecs);
      }

      let audioCodec = '';
      if (variant.audio) {
        audioCodec = MimeUtils.getCodecBase(variant.audio.codecs);
      }

      return videoCodec.substring(0, 3) + '_' + audioCodec.substring(0, 3);
    }
    ...
    variantsByCodecs.forEach((codecs, variants) => {
      for (const variant of variants) {
        ...
        if (exists = commonVariants[variant.video.height]) {
          const existsPriority = codecsPriority[variantCodecs(exists)];
          const variantPriority = codecsPriority[variantCodecs(variant)];
