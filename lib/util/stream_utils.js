  ...
  static chooseCodecsAndFilterManifest(manifest, preferredAudioChannelCount) {
    ...
    variantsByCodecs.forEach((codecs, variants) => {
      for (const variant of variants) {
        let exists;
        if (exists = commonVariants[variant.video.height]) {
          const existsPriority = codecsPriority[
              StreamUtils.getGroupVariantCodecs_(exists)
          ];
          const variantPriority = codecsPriority[
              StreamUtils.getGroupVariantCodecs_(variant)
          ];
          if (existsPriority < variantPriority) {
            continue;
          }
        }
        commonVariants[variant.video.height] = variant;
      }
    });

    const variantsIds = Object.values(commonVariants).map((v) => v.id);
