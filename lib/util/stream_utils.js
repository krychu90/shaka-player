  ...
  static chooseCodecsAndFilterManifest(manifest, preferredVideoCodecs,
    ...
    variantsByCodecs.forEach((codecs, variants) => {
      for (const variant of variants) {
        ...
        const variantPriority = codecsPriority[
        ...
        commonVariants[height].variants.push(variant.id);
      }
    });

    const variantsIds = [];
    for (const v of Object.values(commonVariants)) {
      variantsIds.push(...v.variants);
    }
