...
shaka.util.StreamUtils = class {
  ...
  static chooseCodecsAndFilterManifest(manifest, preferredVideoCodecs,
    ...
    const variantsByCodecs = StreamUtils.getVariantsByCodecs_(variants);
    // variantsByCodecs =
    // StreamUtils.filterVariantsByDensity_(variantsByCodecs);
    ...
    /** @private {shaka.extern.VariantPriority} */
    const codecsPriority_ = {
      av0_opu: 0,
      av1_opu: 0,
      av0_mp4: 1,
      av1_mp4: 1,
    ...
    variantsByCodecs.forEach((codecs, variants) => {
      for (const variant of variants) {
        const height = variant.video ? variant.video.height : 0;
        const variantPriority = codecsPriority_[
            StreamUtils.getVariantCodecs_(variant)
        ];
        let override = false;
        if (commonVariants[height]) {
          const existsPriority = commonVariants[height].priority;
          if (existsPriority < variantPriority) {
            continue;
          }
          if (existsPriority > variantPriority) {
            override = true;
          } else if (!variant.audio || !commonVariants[height].variants
              .some((v) => v.audio.language !== variant.audio.language)) {
            continue;
          }
        } else {
          override = true;
        ...
        if (override) {
          commonVariants[height] = {
            priority: variantPriority,
            variants: [],
          };
        }
        commonVariants[height].variants.push(variant);
      }
    });

    const variantsIds = [];
    for (const h of Object.values(commonVariants)) {
      for (const v of h.variants) {
        variantsIds.push(v.id);
      }
    }
    ...
    manifest.variants = manifest.variants.filter((variant) => {
      return variantsIds.includes(variant.id);
    });
  }
  ...
  */
  ...
  static getVariantCodecs_(variant) {
    ...
    return baseVideoCodec.substring(0, 3) + '_' +
        baseAudioCodec.substring(0, 3);
  }
  ...
  static async filterManifestByMediaCapabilities(
    ...
    manifest.variants = manifest.variants.filter((variant) => {
      ...
      if (video) {
        ...
        if (video.codecs.startsWith('vp09')) {
          /** @private {HTMLMediaElement} */
          const videoEl_ = /** @type {HTMLMediaElement} */
            (document.createElement('video'));
          if (
            videoEl_.canPlayType('video/webm; codecs="vp9"') !== '' &&
            videoEl_.canPlayType(fullType) === ''
          ) {
            variant.video.codecs = 'vp9';
            // eslint-disable-next-line no-alert
            alert(video.codecs + ' -> vp9');
          }
        }
      }
