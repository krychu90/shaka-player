  ...
  static chooseCodecsAndFilterManifest(manifest, preferredVideoCodecs,
    ...
    const getVideoGroupId = (stream) => {
      let id = String(stream.width || '') + String(stream.height || '') +
          String(Math.round(stream.frameRate || 0)) + (stream.hdr || 'SDR') +
    ...
    manifest.variants = manifest.variants.filter((variant) => {
      ...
      if (video) {
        ...
        if (audio) {
          const vCodecs = MimeUtils.getNormalizedCodec(video.codecs);
          const aCodecs = MimeUtils.getNormalizedCodec(audio.codecs);
          if (vCodecs === 'av01' && aCodecs === 'aac') {
            shaka.log.debug('Dropping Variant (AV1 + AAC)', variant);
            return false;
          }
          if (vCodecs === 'avc' && aCodecs === 'opus') {
            shaka.log.debug('Dropping Variant (AVC + OPUS)', variant);
            return false;
          }
        }
      }
